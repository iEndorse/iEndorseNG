...
const HomeCampaign = ({ item }: any, index: any) => {
  ...
  const openInsufficientWalletModal = () => {
    setInsufficientWalletModal(true)
    document.body.style.overflow = "hidden"
  }
  const closeInsufficientWalletModal = () => {
    setInsufficientWalletModal(false)
    document.body.style.overflow = "auto"
  }
  ...
  const openShareCampaignModal = () => {
    setShareCampaignModal(true)
    document.body.style.overflow = "hidden"
  }
  const closeShareCampaignModal = () => {
    setShareCampaignModal(false)
    document.body.style.overflow = "auto"
  }
  const openEndorsementSuccessfulModal = () => {
    setEndorsementSuccessfulModal(true)
    document.body.style.overflow = "hidden"
  }
    ...
    <>
      ...
      >
        ...
        <div className="relative">
          <Carousel
            indicators={item?.campaignFiles.length > 1}
            navButtonsAlwaysVisible={item?.campaignFiles.length > 1}
            navButtonsProps={{
              style: {
                ...
                borderRadius: "50%",
                margin: "0 10px",
                top: "40%", // Position buttons in the middle, away from video controls
              },
            }}
          ...
          >
            {item?.campaignFiles.map((file: any, index: any) => (
              <div
                key={index}
                className="relative flex items-center justify-center bg-black w-full h-[300px] rounded-lg"
              >
                {file.filePath.endsWith(".mp4") ? (
                  ...
                  <img
                    src={file.filePath || "/placeholder.svg"}
                    alt={`Campaign ${index}`}
      ...
      <ShareCampaignModal isOpen={shareCampaignModal} onClose={closeShareCampaignModal} details={item} />

      <SignInFirst isOpen={signInFirstModal} onClose={closeSignInPrompt} />
    </>
  )
}
